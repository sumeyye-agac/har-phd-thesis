"""
Chapter 6 — LM RAB-KD-Att Grid Search
...
Teacher: OM-Att (pre-trained with matching attention config from Chapter 4 grid search)
Student: LM-Att (with same attention config)

Note: This is similar to RB-KD-Att, but uses pre-trained OM-Att models from Chapter 4.
Each experiment uses the OM-Att model that was trained with the same attention config.

All models are saved and results are written to results/grid_search_rabkd_att.csv
"""
...
PARSER_CONFIG = {
    ...
    'help_text': """
LM RAB-KD-Att Grid Search - Usage
...
Examples:
  ...
  python ch6_grid_search_rabkd_att.py
  
  # Run only CH_ATT with temperature 1,2 and alpha 0.1,0.2
  python ch6_grid_search_rabkd_att.py ch_att=true sp_att=false cbam=false temperatures=1,2 alphas=0.1,0.2
  
  # Run with custom attention parameters
  python ch6_grid_search_rabkd_att.py channel_ratios=2,4 layer_positions=1,2

Note: All parameters are case-insensitive. Invalid parameters or values will stop execution.
""",
    'script_name': 'ch6_grid_search_rabkd_att.py'
}
...
def main():
    ...
    for temp, alpha, att_config in product(temperatures, alphas, attention_configs):
        ...
        # Get teacher model name (OM-Att with same attention config - must be pre-trained)
        ...
        # Load teacher each time (no cache to save memory)
        teacher = None
        try:
        ...
        # Create attention lists for student (same as teacher)
        ...
        try:
            ...
            # Clear teacher and student immediately after training
